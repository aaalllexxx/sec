...
Для создания или обновления ключа подписи необходим пароль администратора безопасности:
```sh
python "APM/apm.py" sec add-admin
```
*(Пароль хешируется через PBKDF2 и сохраняется локально).*

### 2. Подпись проекта
Сканирует файлы (`.py`, `.html`, `.js`, `.json`), вычисляет контрольные суммы и генерирует `security.sig`:
```sh
python "APM/apm.py" sec sign
```
**Важно:** После успешной подписи скрипт автоматически устанавливает атрибут **Read-Only (+R)** на `main.py`, `code_signer.py` и `sec_sign.key`.

### 3. Runtime Верификация

...
| `code_signer` | **(NEW)** Проверка целостности кода и защита от инъекций |

---

## 🔐 Электронная подпись и защита целостности (`code_signer`)

Модуль предотвращает несанкционированные изменения кода и инъекции вредоносных файлов. Принцип работы основан на расчете HMAC-SHA256 хешей проекта и автоматической блокировке файлов на уровне ОС.

### 1. Настройка администратора
Для создания или управления ключом подписи необходим пароль администратора безопасности:
```sh
apm sec add_admin
```
*(Пароль хешируется через PBKDF2-SHA256 и сохраняется локально в `.apm/sec/sec_admin.json`).*

### 2. Подпись проекта
Сканирует файлы (`.py`, `.html`, `.js`, `.json`), вычисляет контрольные суммы и генерирует `security.sig`:
```sh
apm sec sign
```
**Важно:** После успешной подписи скрипт автоматически устанавливает атрибуты защиты на `main.py`, `code_signer.py`, `sec_sign.key` и `sec_admin.json`.
- **Windows**: Read-Only (+R), Скрытый (+H), Системный (+S).
- **POSIX**: chmod -w (снятие прав на запись).

### 3. Снятие подписи (Unsigning)
Если необходимо вернуться в режим разработки и разблокировать файлы:
```sh
apm sec unsign
```
Команда запросит пароль администратора, удалит `security.sig` и снимет атрибуты защиты с критических файлов.

### 4. Runtime Верификация
Для активации защиты добавьте вызов в `main.py`:

```python
from AEngineApps.code_signer import verify_project_signature

class ShowcaseApp(App):
    def __init__(self):
        # Проверка целостности ДО запуска сервера
        verify_project_signature()
        super().__init__()
```

Если целостность нарушена, сервер выдаст `CRITICAL` лог и немедленно завершит работу.

### Обновление кода
Чтобы обновить файлы легально:
1. Снимите защиту: `attrib -R main.py`.
2. Внесите изменения.
3. Переподпишите: `apm sec sign`.

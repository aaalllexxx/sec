...
def _flatten_json_names(data) -> list[str]:
    values = []
    if isinstance(data, dict):
        for k, v in data.items():
            values.append(str(k))
            values.extend(_flatten_json_names(v))
    elif isinstance(data, list):
        for item in data:
            values.extend(_flatten_json_names(item))
    return values
...
def _get_request_arg_values() -> list[str]:
    values = [str(v) for v in request.args.values()]

    if request.form:
        values.extend(str(v) for v in request.form.values())
...
def _get_request_arg_names() -> list[str]:
    values = [str(k) for k in request.args.keys()]

    if request.form:
        values.extend(str(k) for k in request.form.keys())

    json_data = _get_json_data()
    if json_data is not None:
        values.extend(_flatten_json_names(json_data))
...
def _values_for_signature_targets(targets: Optional[list[str]]) -> list[str]:
    if not targets:
        return [_get_request_full_data()]

    values = []
    excluded_headers = {
        str(target).split(":", 1)[1].lower()
        for target in targets
        if str(target).upper().startswith("!REQUEST_HEADERS:")
        and ":" in str(target)
    }
    ...
    for raw_target in targets:
        ...
        target_name = target_upper.split(":", 1)[0]
        header_name = target.split(":", 1)[1] if ":" in target else ""

        if target_name in {"ARGS", "ARGS_GET", "ARGS_POST"}:
            values.extend(_get_request_arg_values())
        elif target_name in {"ARGS_NAMES", "ARGS_GET_NAMES", "ARGS_POST_NAMES"}:
            values.extend(_get_request_arg_names())
        elif target_name == "REQUEST_COOKIES":
            values.extend(str(v) for v in request.cookies.values())
        elif target_name == "REQUEST_COOKIES_NAMES":
            values.extend(str(k) for k in request.cookies.keys())
        elif target_name == "REQUEST_FILENAME":
            values.append(request.path)
        elif target_name in {"REQUEST_URI", "REQUEST_URI_RAW"}:
            values.append(request.full_path)
        elif target_name == "REQUEST_BODY":
            values.append(request.get_data(as_text=True))
        elif target_name == "QUERY_STRING":
            values.append(request.query_string.decode("utf-8", errors="ignore"))
        elif target_name == "FILES":
            values.extend(file.filename for file in request.files.values())
        elif target_name == "FILES_NAMES":
            values.extend(str(k) for k in request.files.keys())
            values.extend(file.filename for file in request.files.values())
        elif target_name.startswith("XML"):
            content_type = request.headers.get("Content-Type", "").lower()
            if "xml" in content_type:
                values.append(request.get_data(as_text=True))
        elif target_name == "REQUEST_HEADERS_NAMES":
            values.extend(str(k) for k in request.headers.keys())
        elif target_name == "REQUEST_HEADERS":
            if header_name:
                if header_name.lower() not in excluded_headers:
                    values.append(request.headers.get(header_name, ""))
            else:
                values.extend(
                    str(v)
        ...
        elif target_name == "REQUEST_LINE":
            values.append(f"{request.method} {request.full_path}")
...
class SignatureDetector(BaseDetector):
        ...
        if os.path.exists(db_path):
            try:
                ...
                for sig in db.get("signatures", []):
                    ...
                    targets = sig.get("targets") or sig.get("source_targets") or []
                    if isinstance(targets, str):
                        targets = [target.strip() for target in targets.split("|") if target.strip()]
                    
                    try:
                        compiled = re.compile(pattern, flags)
                        compiled_signatures[name] = compiled
                ...
                if loaded:
                    cls.signatures = compiled_signatures
                    cls._signature_rules = compiled_rules
                else:
                    cls.signatures = dict(cls._builtin_signatures)
                    cls._signature_rules = [
                        {"name": name, "pattern": pattern, "targets": []}
                        for name, pattern in cls.signatures.items()
                    ]
        ...
        try:
            ...
            for sig in db.get("signatures", []):
                ...
                if pattern:
                    ...
                    compiled = re.compile(pattern, flags)
                    cls.signatures[name] = compiled
                    cls._signature_rules.append({
                        "name": name,
                        "pattern": compiled,
                        "targets": targets,
                    })
    ...
    def run(self) -> None:
        for rule in self._signature_rules:
            name = rule["name"]
            pattern = rule["pattern"]

            for value in _values_for_signature_targets(rule.get("targets")):
...
class IPS(IDS):
    ...
    def __init__(self, app):
        ...
        try:
            ...
            if not getattr(sec_config, "MODULES_STATUS", {}).get("intrusion", True):
                return
